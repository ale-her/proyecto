...
public class EnfermerosF extends JFrame {
    ...
    private void initComponents() {
        ...
        tfApM = new JTextField(12);            // apellido materno
        ...
        table.getSelectionModel().addListSelectionListener((ListSelectionListener) ev -> {
            if (!ev.getValueIsAdjusting() && table.getSelectedRow() != -1) {
                ...
                tfApM.setText(model.getValueAt(r,3).toString()); // A. materno
    ...
    private void loadTable() {
        model.setRowCount(0);
        // Asegúrate que la tabla en la BD tenga exactamente estas columnas:
        // codigo_enfermeros, nombre_enfermeros, a_paterno, a_materno, fecha_contratacion, area
        String sql = "SELECT codigo_enfermeros, nombre_enfermeros, a_paterno, a_materno, fecha_contratacion, area FROM enfermeros";
        try (Connection c = conexion.getConnection();
             Statement st = c.createStatement();
             ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) {
                Vector<Object> row = new Vector<>();
                row.add(rs.getString("codigo_enfermeros"));
                row.add(rs.getString("nombre_enfermeros"));
                row.add(rs.getString("a_paterno"));
                row.add(rs.getString("a_materno")); // materno
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error al cargar enfermeros: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void guardar() {
        String sql = "INSERT INTO enfermeros (codigo_enfermeros, nombre_enfermeros, a_paterno, a_materno, fecha_contratacion, area) VALUES (?,?,?,?,?,?)";
        try (Connection c = conexion.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, tfCodigo.getText().trim());
            ps.setString(2, tfNombre.getText().trim());
            ps.setString(3, tfApP.getText().trim());
            ps.setString(4, tfApM.getText().trim()); // materno
    ...
    private void actualizar() {
        ...
        String sql = "UPDATE enfermeros SET nombre_enfermeros=?, a_paterno=?, a_materno=?, fecha_contratacion=?, area=? WHERE codigo_enfermeros=?";
        try (Connection c = conexion.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, tfNombre.getText().trim());
            ps.setString(2, tfApP.getText().trim());
            ps.setString(3, tfApM.getText().trim()); // materno
    ...
    private void eliminar() {
        ...
        String sql = "DELETE FROM enfermeros WHERE codigo_enfermeros=?";
        try (Connection c = conexion.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    private void consultar() {
        ...
        String sql = "SELECT codigo_enfermeros, nombre_enfermeros, a_paterno, a_materno, fecha_contratacion, area FROM enfermeros WHERE codigo_enfermeros=?";
        try (Connection c = conexion.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, codigo);
            try (ResultSet rs = ps.executeQuery()) {
                ...
                if (rs.next()) {
                    ...
                    JOptionPane.showMessageDialog(this, "No existe ese enfermero");
                }
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
